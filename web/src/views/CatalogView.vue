  ...
  <div class="grid">
    <section class="card">
      ...
      <div class="bd">
        ...
        <div class="field">
          <label>
            <input type="checkbox" :checked="state.adult" @change="onToggle('adult', $event)" />
            Adult
          </label>
        ...
        <div class="field">
          <label>
            <input type="checkbox" :checked="state.onlyNew" @change="onToggle('onlyNew', $event)" />
            Только новые (без приквела)
          </label>
          <label>
            <input type="checkbox" :checked="state.onlySequels" @change="onToggle('onlySequels', $event)" />
        ...
        <div class="field">
          ...
          <div class="muted" style="font-size:12px">По умолчанию — любой из выбранных. Если выбраны теги — фильтр учитывает и их. Для режима «все выбранные» включи «Строгое совпадение».</div>
        </div>
        ...
        <div class="field">
          ...
          <div class="muted" style="font-size:12px">
            Enter — добавить тег.
            <span v-if="state.strictMatch">Тайтл должен содержать все выбранные теги.</span>
            <span v-else>Достаточно любого совпадения по выбранным тегам (и жанрам, если они выбраны).</span>
          </div>
        </div>
        <div class="field">
          <label class="checkbox">
            <input type="checkbox" :checked="state.strictMatch" @change="onToggle('strictMatch', $event)" />
            Строгое совпадение
          </label>
          <div class="muted" style="font-size:12px">
            Включено — тайтл должен содержать все выбранные жанры/теги. Выключено — достаточно любого совпадения.
          </div>
        </div>
        ...
        <div v-if="me" class="field">
          ...
          <label style="margin-top:8px">
            <input type="checkbox" :checked="state.notInMyList" @change="onToggle('notInMyList', $event)" />
            Не в моём списке
          </label>
          <label>
            <input type="checkbox" :checked="state.watchLaterOnly" @change="onToggle('watchLaterOnly', $event)" />
            Только «посмотреть позже»
          </label>
          <label>
            <input type="checkbox" :checked="state.showHidden" @change="onToggle('showHidden', $event)" />
            Показывать скрытые
          </label>
        </div>
    ...
    <section>
      ...
      <div v-else class="banner" v-if="catalogMeta">
        ...
        <div
          v-if="state.includeGenres.length || state.includeTags.length || state.strictMatch"
          class="muted"
          style="font-size:12px; margin-top:6px"
        >
          Фильтр: жанры — {{ state.includeGenres.length ? state.includeGenres.join(', ') : '—' }};
          теги — {{ state.includeTags.length ? state.includeTags.join(', ') : '—' }};
          строгое совпадение — {{ state.strictMatch ? 'вкл' : 'выкл' }}
        </div>
      </div>
...
function syncRouteToState() {
  const now = new Date();
  const q = route.query || {};
  const yearRaw = q.year == null ? '' : String(q.year);
  const needsYear = yearRaw.trim() === '';
  const f = parseQueryToFilters(q, { now });

  // Canonicalize URL: always keep a valid year in query to avoid UI "1900" defaults.
  if (needsYear) {
    router.replace({ query: filtersToQuery(f, { now }) });
  }
...
function onToggle(field, e) {
  // In some mobile webviews the route-driven state update can lag a bit.
  // Prefer the DOM checked value when available.
  const next = e?.target ? !!e.target.checked : !state[field];
  commitFilters({ [field]: next });
}
...
function formatGenres(genresJson) {
  ...
  try {
    ...
    if (inc.length) {
      ...
      const incFirst = inc.filter((g) => arr.includes(g));
      const rest = arr.filter((g) => !set.has(g));
      const merged = [...incFirst, ...rest];
      return merged.slice(0, max).join(', ') + (merged.length > max ? ' …' : '');
    }
