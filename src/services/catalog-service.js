...
export function listCatalog(filters, userId) {
  ...
  if (filters.includeGenres?.length) {
    where.push(
      `EXISTS (SELECT 1 FROM media_genres g WHERE g.media_id=m.id AND g.genre IN (${filters.includeGenres.map(() => '?').join(',')}))`
    );
    args.push(...filters.includeGenres);
  }

  if (filters.includeTags?.length) {
    where.push(
      `EXISTS (SELECT 1 FROM media_tags t WHERE t.media_id=m.id AND t.tag_name IN (${filters.includeTags.map(() => '?').join(',')}))`
    );
    args.push(...filters.includeTags);
  }
...
export async function fetchLiveCatalogPage(filters, requestId) {
  ...
  if (filters.includeGenres?.length) {
    const set = new Set(filters.includeGenres);
    out = out.filter((m) => (m.genres || []).some((g) => set.has(g)));
  }
  if (filters.includeTags?.length) {
    const set = new Set(filters.includeTags);
